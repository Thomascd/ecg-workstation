    ...
    partial class MonitorForm : Form
    {
        ...
        private void Record(object sender, EventArgs e)
        {
            ...
            else if (state == state.Record)
            {
                ...
                //StreamWriter temp = new StreamWriter("ECGRecord.temp");
                ...
                {
                    // time fomat 0:00.000
                    double time = Data[0].Time[i] - Data[0].Time[StartRecordPoint];
                    temp.Append((int)time / 60000 + ":"); time = time % 60000;
                    temp.Append((int)time / 10000); time = time % 10000;
                    temp.Append((int)time / 1000 + "."); time = time % 1000;
                    temp.Append((int)time / 100); time = time % 100;
                    temp.Append((int)time / 10); time = time % 10;
                    temp.Append((int)time + "\t");

                    foreach (ECGData _data in Data) temp.Append(_data.Sign[i] + "\t");
                }

                StreamWriter file = new StreamWriter("ECGREcord.temp");
                file.Write(temp.ToString());
                file.Flush();
                file.Close();

                AddNewRecord(temp.ToString());
            }
        }
                ...
                if (StateValue != value)
                {
                    ...
                    switch (value)
                    {
                        case state.History:
                            ...
                            {
                                MessageBox.Show("Can't sent stop messenger to server");
                            }
                            break;
                        ...
                        case state.Realtime:
                            if (StateValue != state.Record)
                            {
                                ...
                                try
                                {
                                    for (int i = 0; i < 12; i++) Flag[i] = true;

                                    UDPSender.IPs = Setting.IDString;
                                    UDPSender.readySend(this.Flag);
                                }
                                catch
                                {
                                    MessageBox.Show("Can't sent ready messenger to server");
                                }
                            }
                            break;
        ...
        void StartSimulator(object sender, EventArgs e)
        {
            ...
            if (ok)
            {
                DataReciveTimer.Interval = SimulatorControler.Cycle;

                DataReciveTimer.Interval = (int)(SimulatorData[0].Time[1] - SimulatorData[0].Time[0]);
                DataReciveTimer.Enabled = true;
